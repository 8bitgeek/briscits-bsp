...
{
	uint32_t afr = port->AFR[(pin<8)?0:1];
	afr &= ~PIN_AFR_MASK(pin<8?pin:pin-8);
	afr |= PIN_AFR(pin<8?pin:pin-8,afr);
	port->AFR[(pin<8)?0:1] = afr;
}

extern void hw_pin_set_mode( GPIO_TypeDef* port, uint8_t pin, uint32_t mode)
{
	uint32_t moder = port->MODER;
	moder &= ~PIN_MODE_MASK(pin);
	moder |= PIN_MODE(pin,mode);
	port->MODER = moder;
}
