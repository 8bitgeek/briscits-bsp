...
static void gadget0_ss_in_cb(usbd_device *usbd_dev, uint8_t ep)
{
	...
	if (x != BULK_EP_MAXPACKET) {
		xfprintf(xstderr,"failed to write?: %d\n", x);
	}
	/*assert(x == sizeof(buf));*/
}
...
static void gadget0_out_cb_loopback(usbd_device *usbd_dev, uint8_t ep)
{
	...
	xfprintf(xstderr,"loop OUT %x got %d => %d\n", ep, x, y);
}
	...
	usbd_control_complete_callback *complete)
{
	...
	(void) complete;
	(void) buf;
	xfprintf(xstderr,"ctrl breq: %x, bmRT: %x, windex :%x, wlen: %x, wval :%x\n",
	...
	case GZ_REQ_SET_ALIGNED:
		...
		return USBD_REQ_HANDLED;
	case GZ_REQ_PRODUCE:
		xfprintf(xstderr,"fake loopback of %d\n", req->wValue);
		if (req->wValue > sizeof(usbd_control_buffer)) {
			xfprintf(xstderr,"Can't write more than out control buffer! %d > %d\n",
				req->wValue, sizeof(usbd_control_buffer));
			return USBD_REQ_NOTSUPP;
		}
		/* Don't produce more than asked for! */
		if (req->wValue > req->wLength) {
			xfprintf(xstderr,"Truncating reply to match wLen\n");
			*len = req->wLength;
		} else {
			*len = req->wValue;
		}
		return USBD_REQ_HANDLED;
	default:
		xfprintf(xstderr,"Unhandled request!\n");
		return USBD_REQ_NOTSUPP;
	}
...
static void gadget0_set_config(usbd_device *usbd_dev, uint16_t wValue)
{
	...
	case GZ_CFG_LOOPBACK:
		...
		break;
	default:
		xfprintf(xstderr,"set configuration unknown: %d\n", wValue);
	}
}
